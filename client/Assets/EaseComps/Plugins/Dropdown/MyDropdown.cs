...
namespace EaseComps.UI
{
    [Serializable]
    public class MyDropdownOption
    {
        ...
        public MyDropdownOption(string nameText)
        {
    ...
    public class MyDropdown : MonoBehaviour
    {
        ...
        [SerializeField]
        AnimationDropdownTypes dropdownAnimationType = AnimationDropdownTypes.Shrinking; //Currently selected animation

        GameObject firstObj;

        public List<MyDropdownOption> optionsList = new List<MyDropdownOption>(); //List of all options

        List<MyDropdownOptionScript> spawnedList = new List<MyDropdownOptionScript>(); //List of all spawned options
        ...
        [SerializeField] MyDropdownAutoSizeLayout optionsDropdown; //Options dropdown resizer
        ...
        void Update()
        {
            ...
            switch (dropdownAnimationType)
            {
                case AnimationDropdownTypes.Shrinking:
                    ratioShrinking =
                        Mathf.Clamp(
                            ratioShrinking + (speedOfShrinking * Time.deltaTime) *
                            curveShrinking.Evaluate(ratioShrinking), 0, 1);
                    optionsObject.sizeDelta = new Vector2(optionsObject.sizeDelta.x,
                        Mathf.Lerp(startPos, targetPos, ratioShrinking));
        ...
        {
            optionsList.Add(new MyDropdownOption(targetOption));
        }
        ...
        public void AddOptions(string[] targetOptions)
        {
            ...
            {
                optionsList.Add(new MyDropdownOption(targetOptions[i]));
            }
        }
        ...
        public void OpenOptions()
        {
            ...
            {
                var newOption = Instantiate(firstObj, optionsContent).GetComponent<MyDropdownOptionScript>();
                newOption.gameObject.SetActive(true);
                newOption.InitOption(i, optionsList[i].nameText);
                spawnedList.Add(newOption);
            }

            optionsObject.GetChild(0).GetComponent<MyDropdownAutoSizeLayout>().UpdateAllRect();
        ...
        IEnumerator WaitForSeveralFrames()
        {
            ...
            if (dropdownAnimationType == AnimationDropdownTypes.None ||
                dropdownAnimationType == AnimationDropdownTypes.Fading)
            {
                optionsObject.sizeDelta = new Vector2(optionsObject.sizeDelta.x,
                    optionsObject.GetChild(0).GetComponent<RectTransform>().sizeDelta.y);
            }

            if (dropdownAnimationType == AnimationDropdownTypes.Shrinking ||
                dropdownAnimationType == AnimationDropdownTypes.None)
            {
