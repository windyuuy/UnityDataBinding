...
{
    [ExecuteInEditMode]
    public class MyDropdownAutoSizeLayout : MonoBehaviour
    {
        ...
        public void UpdateAllRect()
        {
            ...
            for (int i = 0; i < transform.childCount; i++)
            {
                if (transform.GetChild(i).gameObject.activeSelf)
                {
                    ...
                    if (rect.GetComponent<MyDropdownAutoSizeLayout>())
                    {
                        rect.GetComponent<MyDropdownAutoSizeLayout>().UpdateAllRect();
                    }
