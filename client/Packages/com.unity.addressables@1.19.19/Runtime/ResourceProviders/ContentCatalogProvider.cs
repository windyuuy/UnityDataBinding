...
namespace UnityEngine.AddressableAssets.ResourceProviders
{
    ...
    public class ContentCatalogProvider : ResourceProviderBase
    {
        ...
        public bool IsRemoteCatalogInBundle = false;

        public static Func<string, bool> CanLoadFromBundleFunc;
        ...
        internal class InternalOp
        {
            ...
            private bool m_IsRemoteCatalogInBundle;

            public void Start(ProvideHandle providerInterface, bool disableCatalogUpdateOnStart, bool isLocalCatalogInBundle, bool isRemoteCatalogInBundle)
            {
                // Debug.Log("ContentCatalogProvider:Start");
                ...
                m_IsRemoteCatalogInBundle = isRemoteCatalogInBundle;
                ...
                {
                    loadCatalogFromLocalBundle = (isRemoteCatalogInBundle && CanLoadFromBundleFunc != null && CanLoadFromBundleFunc(idToLoad));
                    if (loadCatalogFromLocalBundle)
                    {
                        idToLoad = idToLoad.Replace(kCatalogExt, ".bundle");
                    }
                }
            ...
            {
                Debug.Log($"LoadCatalogFrom: {idToLoad}, {loadCatalogFromLocalBundle}");
                try
                {
                    ...
                    if (loadCatalogFromLocalBundle)
                    {
                        ...
                        {
                            // Debug.Log("m_BundledCatalog.OnLoaded");
            ...
            internal class BundledCatalog
            {
                ...
                public void LoadCatalogFromBundleAsync()
                {
                    ...
                    else
                    {
                        ...
                        m_LoadBundleRequest.completed += loadOp =>
                        {
                            ...
                            {
                                Addressables.LogError($"Unable to load dependent bundle from location1 : {m_BundlePath}");
                ...
                private void WebRequestOperationCompleted(AsyncOperation op)
                {
                    ...
                    {
                        Addressables.LogError($"Unable to load dependent bundle from location2 : {m_BundlePath}");
                        m_OpInProgress = false;
                    }
                ...
                void LoadTextAssetRequestComplete(AsyncOperation op)
                {
                        ...
                        && textAsset.text != null)
                    {
                        ...
                        var t2 = DateTime.Now;
                        Debug.Log($"CatalogDeserailizeTimeCost: {(t2-t1).TotalMilliseconds}");
                        OnLoaded?.Invoke(m_CatalogData);
                        var t3 = DateTime.Now;
                        Debug.Log($"CatalogOnLoadTimeCost: {(t2-t1).TotalMilliseconds}, {(t3-t2).TotalMilliseconds}");
                    }
                    else
                    {
            ...
            private void OnCatalogLoaded(ContentCatalogData ccd)
            {
                ...
                else
                {
                    ...
                    if (!m_Retried)
                    {
                        ...
                        Start(m_ProviderInterface, m_DisableCatalogUpdateOnStart, m_IsLocalCatalogInBundle, m_IsRemoteCatalogInBundle);
                    }
                    else
                    {
        ...
        public override void Provide(ProvideHandle providerInterface)
        {
            ...
            m_LocationToCatalogLoadOpMap[providerInterface.Location].Start(providerInterface, DisableCatalogUpdateOnStart, IsLocalCatalogInBundle, IsRemoteCatalogInBundle);
        }
    }
}
