    ...
    public class AssetBundleResource : IAssetBundleResource, IUpdateReceiver
    {
        ...
        private bool WaitForCompletionHandler()
        {
            ...
            // {
                ...
                var requestOperation = m_RequestOperation;
                ...
                if (requestOperation is UnityWebRequestAsyncOperation op)
                {
                    // Debug.Log($"IsAssetBundleDownloaded-A: {op.GetHashCode()}, {op.webRequest.GetHashCode()}, {this.m_TransformedInternalId}, {m_Source}");
                    while (!UnityWebRequestUtilities.IsAssetBundleDownloaded(op, this.m_TransformedInternalId))
...
#endif
                    // Debug.Log($"IsAssetBundleDownloaded2: {op.webRequest.url}, {m_RequestCompletedCallbackCalled}, {m_Source}, {m_AssetBundle != null}, {op.webRequest.disposeDownloadHandlerOnDispose}, {isTimeout}, {op.isDone}");
                ...
                // Debug.Log($"requestOperation.HashCode: {requestOperation?.GetHashCode()}, {m_TransformedInternalId}");
                if (!m_Completed && m_Source == BundleSource.Local)
                {
                    ...
                    if (!m_RequestCompletedCallbackCalled)
                    {
                        if (requestOperation != null)
                        {
                            ...
                            LocalRequestOperationCompleted(requestOperation);
                        }
                        else
                        {
                            // Debug.Log($"requestOperation is null1: {m_TransformedInternalId}");
                        }
                    }
                }

                // Debug.Log($"Request-Pre-End: {m_TransformedInternalId}, {m_Completed}, {m_RequestOperation?.GetHashCode()}, {requestOperation?.isDone}, {m_RequestOperation?.isDone}, {m_AssetBundle != null},>>");
                if (!m_Completed && requestOperation!=null && requestOperation.isDone 
                    // timeout and cause not loaded
                    && (!(IsTimeOut && m_AssetBundle==null)))
                {
                    // Debug.Log($"m_ProvideHandle.Complete: {m_TransformedInternalId}");
                    m_ProvideHandle.Complete(this, m_AssetBundle != null, null);
                    m_Completed = true;
                }
                // Debug.Log($"Request-End: {m_TransformedInternalId}, {m_RequestOperation?.GetHashCode()}");
            // }
            // catch (Exception ex)
            // {
            //     Debug.LogError($"throw exception: {m_TransformedInternalId}");
            //     throw ex;
            // }
...
// #if UNITY_EDITOR
            Debug.Log($"AAAB-CreateWebRequest: {m_TransformedInternalId}, {loadType}, {this.GetHashCode()}, {m_Options.UseCrcForCachedBundle}");
// #endif
...
#endif
                {
                    var ignoreCrc = m_Options is not { UseCrcForCachedBundle: true };
                    m_RequestOperation = AssetBundle.LoadFromFileAsync(m_TransformedInternalId, ignoreCrc ? 0 : m_Options.Crc);
                    // Debug.Log($"set m_RequestOperation={m_RequestOperation?.GetHashCode()}, {m_TransformedInternalId}");
#if ENABLE_ADDRESSABLE_PROFILER
            ...
            else if (loadType == LoadType.Web)
            {
...
#if ENABLE_ASYNC_ASSETBUNDLE_UWR
                // Debug.Log($"set-autoLoadAssetBundle: {m_TransformedInternalId}, {m_ProvideHandle.Location is DownloadOnlyLocation}");
                ...
                // Debug.Log($"m_WebRequestQueueOperation.IsDone: {m_WebRequestQueueOperation.IsDone}, {m_TransformedInternalId}");
            ...
            else
            {
                ...
                m_ProvideHandle.Complete<AssetBundleResource>(null, false,
                    new RemoteProviderException(string.Format("Invalid path in AssetBundleProvider1: '{0}'.", m_TransformedInternalId), m_ProvideHandle.Location));
                m_Completed = true;
            }
        }
        ...
        {
            // Debug.Log($"BeginWebRequestOperation: {m_TransformedInternalId}");
            IsTimeOut = false;
            m_TimeoutTimer = 0;
            m_TimeoutOverFrames = 0;
            m_LastDownloadedByteCount = 0;
            // Debug.Log($"set m_RequestOperation={asyncOp.GetHashCode()}, {m_WebRequestQueueOperation.IsDone}, {m_TransformedInternalId}");
        ...
        public void Update(float unscaledDeltaTime)
        {
            if (m_RequestOperation != null && m_RequestOperation is UnityWebRequestAsyncOperation operation && !operation.isDone)
            {
                if (m_LastDownloadedByteCount != operation.webRequest.downloadedBytes)
                {
                    // if (IsTimeOut)
                    // {
                    //     Debug.Log($"stop-too-early, IsTimeOut=false: {m_TransformedInternalId}");
                    // }
                    // IsTimeOut = false;
                    m_TimeoutTimer = 0;
                    m_TimeoutOverFrames = 0;
                ...
                else
                {
                    ...
                    {
                        IsTimeOut = true;
                        Debug.LogWarning($"ABDownload-Timeout-Abort: {m_Source}, {operation.webRequest.url}");
                        // if (operation.webRequest.isDone)
                        // {
                        //     Debug.LogError($"operation.webRequest.isDone: {operation.webRequest.url}");
                        // }
                        //
                        // if (operation.webRequest.downloadHandler is DownloadHandlerAssetBundle downloadHandlerAssetBundle)
                        // {
                        //     if (downloadHandlerAssetBundle.isDownloadComplete || downloadHandlerAssetBundle.isDone || downloadHandlerAssetBundle.assetBundle != null)
                        //     {
                        //         Debug.LogError($"downloadHandlerAssetBundle.isDone2: {operation.webRequest.url}, {downloadHandlerAssetBundle.isDownloadComplete} || {downloadHandlerAssetBundle.isDone} || {downloadHandlerAssetBundle.assetBundle != null}");
                        //     }
                        //     // downloadHandlerAssetBundle.autoLoadAssetBundle = false;
        ...
        private void CompleteBundleLoad(AssetBundle bundle)
        {
            ...
            if (m_AssetBundle != null)
                ...
                m_ProvideHandle.Complete<AssetBundleResource>(null, false,
                    new RemoteProviderException(string.Format("Invalid path in AssetBundleProvider2: '{0}'.", m_TransformedInternalId), m_ProvideHandle.Location));
            m_Completed = true;
        }
        ...
        private void WebRequestOperationCompleted(AsyncOperation op)
        {
            ...
            if (webReq != null && !UnityWebRequestUtilities.RequestHasErrors(webReq, out uwrResult, isTimeout))
            {
                if (!m_Completed)
                {
                    Debug.Log($"ABDownloaded: {webReq.url}, {m_Source}, {isTimeout}, {m_ProvideHandle.Location is DownloadOnlyLocation}, {webReq.error}, >>");
#if ENABLE_ADDRESSABLE_PROFILER
                    AddBundleToProfiler(Profiling.ContentStatus.Active, m_Source);
#endif
                    // Debug.Log($"downloadHandler.Dispose: {webReq.url}, {downloadHandler?.assetBundle!=null}");
                    ...
                    else
                    {
                        ...
                        {
                            downloadHandler.assetBundle.Unload(true);
                        }
                    }
                    ...
                    m_ProvideHandle.Complete(this, true, null);
                    m_Completed = true;
                    // Debug.Log($"m_Completed = true: {webReq.url}");
                }
#if ENABLE_CACHING
			...
			else
            {
                ...
                if (isTimeout)
                {
                    // Debug.Log($"isTimeout: {m_TransformedInternalId}");
                    uwrResult.Error = "Request timeout";
                }

                downloadHandler = webReq.downloadHandler as DownloadHandlerAssetBundle;
                // Debug.Log($"downloadHandler.Dispose-error: {webReq.url}, {downloadHandler?.assetBundle!=null}, {isTimeout}, {m_WebRequestQueueOperation?.IsDone}, {op?.isDone}, {webReq?.isDone},>>");
                if (downloadHandler?.assetBundle != null)
                {
                    downloadHandler.assetBundle.Unload(true);
                }
                ...
                string message2 = $"Web request failed, ({m_Retries}/{m_Options.RetryCount})...\n{uwrResult}";
                Debug.Log(message2);
#if ENABLE_CACHING
                if (!string.IsNullOrEmpty(m_Options.Hash))
                {
...
#endif
                    {
                        var message = $"{m_TransformedInternalId}, Web request failed to load from cache. The cached AssetBundle will be cleared from the cache and re-downloaded. Retrying...\n{uwrResult}";
                        ...
                        {
                            // Debug.LogError($"stop force retry: {webReq.url}, {uwrResult.ShouldRetryDownloadError()}, {m_Retries}/{m_Options.RetryCount}, {uwrResult}");
                        }
                    }
                }
#endif
                if (!forcedRetry)
                {
                    if ((m_Retries < m_Options.RetryCount || m_Options.RetryCount <= 0) && uwrResult.ShouldRetryDownloadError())
                    {
                        Debug.LogWarning($"retry by times: {webReq.url}, {m_Source}, {m_Retries}/{m_Options.RetryCount}");
                        m_Retries++;
                        // Debug.LogFormat(message);
                        BeginOperation();
                    }
                    ...
                    {
                        var exception = new RemoteProviderException($"Unable to load asset bundle from2 : {webReq.url}, {m_Retries}/{m_Options.RetryCount}", m_ProvideHandle.Location, uwrResult);
            ...
            // Debug.Log($"webReq.Dispose: {webReq.url}, {op?.GetHashCode()}, {webReq.GetHashCode()}");
            webReq.Dispose();
        }
...
#endif
        {
            Debug.Log($"AAUnloadAssetBundle: {this.m_TransformedInternalId}, {this.m_Source}, {(m_AssetBundle != null ? m_AssetBundle.name : "null")}");
#if UNLOAD_BUNDLE_ASYNC
            unloadOp = null;
            if (m_AssetBundle != null)
            {
