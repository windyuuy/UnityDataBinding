    ...
    public class BundledAssetProvider : ResourceProviderBase
    {
        internal class InternalOp
        {
            ...
            public void Start(ProvideHandle provideHandle)
            {
                ...
                {
                    m_ProvideHandle.Complete<AssetBundle>(null, false, new Exception("Unable to load dependent bundle from location3 " + m_ProvideHandle.Location));
                }
                else
                {
                    ...
                    {
                        m_ProvideHandle.Complete<AssetBundle>(null, false, new Exception("Unable to load dependent bundle from location4 " + m_ProvideHandle.Location));
                        return;
                    }
            ...
            private void BeginAssetLoad()
            {
                if (m_AssetBundle == null)
                {
                    m_ProvideHandle.Complete<AssetBundle>(null, false, new Exception("Unable to load dependent bundle from location5 " + m_ProvideHandle.Location));
                }
                else
                {
