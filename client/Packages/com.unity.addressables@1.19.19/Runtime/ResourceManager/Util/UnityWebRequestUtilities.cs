...
namespace UnityEngine.ResourceManagement.Util
{
    ...
    public class UnityWebRequestUtilities
    {
        ...
        public static bool RequestHasErrors(UnityWebRequest webReq, out UnityWebRequestResult result, bool timeout=false)
        {
            result = null;
            if (webReq == null || !webReq.isDone)
            {
                if (timeout)
                {
                    ...
                    // {
                        ...
                        // Debug.Log($"return timeoutxcxx2: {webReq.url}");
                    // }
                    // catch (Exception ex)
                    // {
                    //     // Debug.LogError("create timeout result failed");
                    //     throw ex;
                    // }
                    return true;
                }
                ...
                {
                    // Debug.Log($"webReq is invalid: {webReq == null}, {webReq == null || !webReq.isDone}");
                }
                return false;
            }
                ...
                case UnityWebRequest.Result.Success:
                    if (timeout)
                    {
                        ...
                        // {
                            result = new UnityWebRequestResult(webReq);
                            result.Error = "Request timeout";
                            // Debug.Log($"return timeoutxcxx: {webReq.url}");
                        // }
                        // catch (Exception ex)
                        // {
                        //     Debug.LogError("create timeout result failed");
        ...
        public static bool IsAssetBundleDownloaded(UnityWebRequestAsyncOperation op, string m_TransformedInternalId=null)
        {
...
// #if DEVELOPMENT_BUILD
                ...
                try
                {
                    url = op.webRequest.url;
                }
                catch (Exception)
                {
                    url = m_TransformedInternalId;
                }
                Debug.LogWarning($"op.webRequest is disposed: {url}");
// #endif
                return true;
            }
#endif
